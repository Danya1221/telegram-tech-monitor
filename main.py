...
def analytics_row_raw_text(row) -> str:
    """
    asyncpg.Record-safe access for v11 raw_text.
    """
    try:
        return (
            row["raw_text"]
...
def analytics_interest_match(
    ...
    """
    Match an interest against the original source message first.

    Old rows that do not have raw_text are still supported via
    found_request.
    """
    source = (
        raw_text
...
async def interesting_analytics_text(
    ...
    # Каждое реальное сообщение-запрос считаем максимум ОДИН раз:
    # присваиваем его лучшей позиции из списка интересующих.
...
async def monitor_handler(event, account_id: int):
    try:
        ...
        # v11 hard fallback:
        # if the generic analytics detector missed a real request,
        # check the original message directly against analytics interests.
        if not analytics_request:
            (
                analytics_request,
                direct_interest_score,
            ) = await direct_interest_match_for_analytics(
                owner_id,
                text,
            )
            ...
            if analytics_request:
                print(
                    "INTEREST DIRECT ANALYTICS | "
                    f"request={analytics_request!r} | "
                    f"score={direct_interest_score:.0f} | "
                    f"chat={chat_id} | "
                    f"message={event.id}"
                )
