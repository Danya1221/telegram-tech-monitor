...
async def acquire_single_instance_lock():
    """Only one Quantum process may poll Bot API for this PostgreSQL DB."""
    ...
    try:
        ...
        if not locked:
            raise RuntimeError(
                'Другой экземпляр Quantum уже запущен с этой БД.'
            )
...
async def release_single_instance_lock():
    ...
    if conn is None:
        return
    try:
        await conn.execute('SELECT pg_advisory_unlock(713202614)')
    except Exception:
        pass
    try:
        await db_pool.release(conn)
...
async def main():
    await init_db()
    # V18: no process-lifetime PostgreSQL advisory lock.
    # Railway rolling deploy starts the new container before stopping the old
    # one; the old lock made the new deployment crash before Bot API polling.
