...
_owner_id_cache = ADMIN_ID
_owner_id_cache_loaded = ADMIN_ID is not None
_queries_cache: dict[int, list] = {}
_interest_queries_cache: dict[int, list] = {}
...
def invalidate_query_cache(owner_id: int):
    _queries_cache.pop(owner_id, None)


def invalidate_interest_cache(owner_id: int):
    _interest_queries_cache.pop(owner_id, None)
...
def invalidate_accounts_cache(owner_id: int):
    _accounts_cache.pop(owner_id, None)
...
async def get_owner_id() -> int | None:
    ...
    global _owner_id_cache_loaded

    if ADMIN_ID:
        return ADMIN_ID

    if _owner_id_cache_loaded:
        return _owner_id_cache

    value = await get_setting("owner_id")
    _owner_id_cache = int(value) if value else None
    _owner_id_cache_loaded = True
    return _owner_id_cache
...
async def get_queries(owner_id: int):
    """Горячий список поиска: читаем PostgreSQL один раз до изменения."""
    cached = _queries_cache.get(owner_id)
    ...
    for row in rows:
        key = canonical_query_key(row["query"])
    ...
    _queries_cache[owner_id] = result
    return result
...
async def add_query(owner_id: int, query: str) -> bool:
    ...
    invalidate_query_cache(owner_id)
    return True
...
async def add_queries(
    ...
    """Добавляет много строк одним чтением/одним executemany."""
    entries = parse_query_entries(raw_text)

    if not entries:
        return 0, []

    existing_rows = await get_queries(owner_id)
    existing_keys = {
        canonical_query_key(row["query"])
    ...
    for query in entries:
        ...
        if not key or key in existing_keys or key in added_keys:
            continue
    ...
    async with db_pool.acquire() as conn:
        await conn.executemany(
            """
            INSERT INTO monitor_queries_v2 (owner_id, query)
            VALUES ($1, $2)
            """,
            [(owner_id, query) for query in added],
        )
...
async def get_interest_queries(
    ...
    cached = _interest_queries_cache.get(owner_id)
    ...
    _interest_queries_cache[owner_id] = result
    return result
...
async def interest_query_count(
    ...
    return len(await get_interest_queries(owner_id))
...
async def get_interest_queries_page(
    ...
    rows = await get_interest_queries(owner_id)
    total = len(rows)
...
async def add_interest_query(
    ...
    invalidate_interest_cache(owner_id)
    return True
...
async def add_interest_queries(
    ...
    existing_rows = await get_interest_queries(owner_id)
    existing_keys = {
        canonical_query_key(row["query"])
        for row in existing_rows
    }
    ...
    async with db_pool.acquire() as conn:
        await conn.executemany(
            """
            INSERT INTO interest_queries_v1 (owner_id, query)
            VALUES ($1, $2)
            """,
            [(owner_id, query) for query in added],
        )
...
async def delete_interest_query(
    ...
    rows = await get_interest_queries(owner_id)
    selected = next(
        (row for row in rows if int(row["id"]) == int(query_id)),
        None,
    )
    ...
    selected_query = str(selected["query"])
    selected_key = canonical_query_key(selected_query)
    ids_to_delete = [
        int(row["id"])
        for row in rows
        if canonical_query_key(row["query"]) == selected_key
    ]
...
async def get_accounts(owner_id: int):
    cached = _accounts_cache.get(owner_id)
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            """
            SELECT
                id, owner_id, label, session, tg_user_id,
                username, first_name, active, created_at
    ...
    result = list(rows)
    _accounts_cache[owner_id] = result
    return result
...
async def account_count(owner_id: int) -> int:
    return len(await get_accounts(owner_id))
...
async def connect_saved_account(row) -> bool:
    ...
    try:
        ...
        invalidate_accounts_cache(int(row["owner_id"]))
...
async def get_all_dialogs(
    ...
    """
    Общий список чатов обоих аккаунтов. Telegram API — дорогой вызов,
    поэтому обычная навигация использует кэш 120 секунд.
    Кнопка «Обновить» делает принудительный refresh.
    """
    now = time.monotonic()
    cached = _dialogs_cache.get(owner_id)
    ...
    active_rows = [
        ...
        if int(row["id"]) in clients
    ]

    if not active_rows:
        _dialogs_cache[owner_id] = (now, [])
        return []

    dialog_lists = await asyncio.gather(
        *(
            get_dialogs_for_account(int(row["id"]))
    ...
    for row, dialogs in zip(active_rows, dialog_lists):
        ...
        for dialog in dialogs:
            item = dict(dialog)
            item["account_index"] = account_position.get(account_id, 1)
    ...
    _dialogs_cache[owner_id] = (time.monotonic(), result)
    return result
...
async def status_text(owner_id: int):
    ...
    for index, row in enumerate(account_rows, start=1):
        ...
        username = row["username"]
        first_name = row["first_name"]
        name = f"@{username}" if username else (first_name or row["label"])
        connected_lines.append(
            f"{'🟢' if connected else '🔴'} {index}. {name}"
        )

    if not connected_lines:
        connected_lines = ["🔴 Telegram-аккаунты не подключены"]
...
async def main():
    ...
    try:
        ...
        for task in [maintenance_task, maintenance_loop_task]:
