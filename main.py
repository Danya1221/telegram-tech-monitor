...
# Только список Telegram-чатов кэшируется в RAM.
# Профили/запросы/настройки по-прежнему читаются из PostgreSQL.
#
# Причина: iter_dialogs(limit=500) по двум аккаунтам — дорогой Telegram API вызов.
# Перелистывание страниц не должно повторять его снова и снова.
dialogs_cache: dict[int, dict] = {}

# Dedicated PostgreSQL connection that holds the single-instance lock.
...
def instance_lock_key() -> int:
    """Stable positive 63-bit advisory-lock key derived from BOT_TOKEN."""
...
async def acquire_instance_lock():
    """
    Prevent two copies of this bot from using the same PostgreSQL/BOT_TOKEN.
    This is the main protection against alternating stale menus and duplicate
    Telethon sessions when Railway accidentally runs two replicas/services.
    """
    global instance_lock_conn

    instance_lock_conn = await asyncpg.connect(
        DATABASE_URL
    )

    locked = await instance_lock_conn.fetchval(
    ...
    if not locked:
        ...
        raise RuntimeError(
            ...
            "with the same BOT_TOKEN/DATABASE_URL. "
            "Leave only one Railway service/replica."
        )
...
async def init_db():
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_queries_owner_id
            ON queries (owner_id, id DESC)
        """)
        ...
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_accounts_owner_active
...
async def connect_saved_account(row) -> bool:
    ...
    try:
        ...
        if not authorized:
            ...
            # Telegram explicitly says this StringSession is not authorized.
            # This is not a normal network disconnect.
            async with db_pool.acquire() as conn:
                await conn.execute(
                    """
                    UPDATE telegram_accounts
        ...
        # A temporary network error must NOT remove the profile from DB.
        try:
            await tg_client.disconnect()
        except Exception:
            pass
...
async def load_saved_clients(owner_id: int):
    """Connect both saved Telegram accounts in parallel."""
...
async def reconnect_missing_accounts_loop(
    owner_id: int,
):
    """
    Lightweight safety net.
    It never probes/disconnects a healthy account.
    It only retries profiles that are still active in PostgreSQL but whose
    runtime client/task is missing or already stopped.
    """
    while True:
...
def best_query_segment(
    ...
    """
    Возвращает:
      score, самый релевантный кусок сообщения

    Если товар и характеристика находятся на одной строке,
    в уведомление уйдёт только эта строка, а не весь прайс.

    Если название и характеристика разбиты на две соседние строки,
    дополнительно проверяем двухстрочное окно.
    """
    ...
    # Иногда продавец пишет:
    # Samsung S26
    # Black 256GB
    #
    # Тогда две строки нужно рассматривать вместе.
    for index in range(
        len(parts) - 1
    ):
        combined = (
    ...
    for candidate in candidates:
        ...
        # При одинаковом score оставляем первый вариант.
        # Однострочные кандидаты идут раньше двухстрочных,
        # поэтому уведомление будет максимально коротким.
        if score > best_score:
            best_score = score
...
async def get_all_dialogs(
    ...
    Первый вход / ручной Refresh:
      PostgreSQL -> оба Telethon аккаунта -> iter_dialogs()

    Перелистывание / поиск / выбор:
      берём уже готовый список из RAM, Telegram повторно НЕ опрашиваем.

    В RAM хранится только UI-список чатов.
    Профили, запросы и выбранные чаты остаются PostgreSQL-данными.
    """
    ...
    if (
        ...
        and cached.get("account_ids") == active_account_ids
    ):
    ...
    if not online_rows:
        # Если Telegram временно переподключается, не уничтожаем
        # последний нормальный список чатов.
    ...
    for row, dialogs in zip(
        ...
        if isinstance(dialogs, Exception):
            print(
                "DIALOGS ERROR | "
                f"account_id={int(row['id'])} | "
                f"{dialogs!r}"
            )
            continue
    ...
    # Если один аккаунт временно не ответил, не заменяем хороший
    # полный кэш урезанным списком только одного аккаунта.
    if (
        cached is not None
        and len(online_rows) < len(account_rows)
        and len(cached["dialogs"]) > len(result)
    ):
        return cached["dialogs"]
...
async def queries_text(owner_id: int):
    ...
    for row in rows:
        ...
        # Keep a safety margin below Telegram's 4096-char limit.
        if current_len + len(line) + 1 > 3700:
            lines.append(
                "… список длинный; остальные запросы есть в PostgreSQL."
            )
            break
...
async def build_query_delete_keyboard(
    ...
    start = page * QUERY_DELETE_PAGE_SIZE
    current = rows[
        start:start + QUERY_DELETE_PAGE_SIZE
    ]
    ...
    for row in current:
        ...
        keyboard.append([
            InlineKeyboardButton(
                text=f"❌ {title}",
                callback_data=(
                    f"query:remove:{int(row['id'])}:{page}"
                ),
            )
        ])
    ...
    if page > 0:
        nav.append(
            InlineKeyboardButton(
                text="◀️",
                callback_data=f"query:delete:page:{page - 1}",
            )
        )
...
@dp.callback_query(F.data.startswith("query:delete"))
...
async def build_chats_keyboard(
    ...
    rows.append([
        InlineKeyboardButton(
            text="🔄 Обновить",
            callback_data=f"chats:refresh:{page}",
        )
    ])
...
async def render_chats(
    ...
    if not dialogs:
        text = (
            "💬 Telegram пока не вернул группы/каналы.\n\n"
    ...
    text = (
        ...
        "Страницы листаются без повторной загрузки Telegram."
    )
...
@dp.callback_query(
    F.data.startswith("chats:refresh:")
)
...
async def main():
    ...
    try:
        ...
        # Split old multiline query packages into independent DB rows.
        ...
        if owner_id is not None:
            ...
            reconnect_task = asyncio.create_task(
                reconnect_missing_accounts_loop(
                    owner_id
                )
            )
        ...
        # Remove an old webhook and stale Bot API updates from previous deploys.
        # This prevents old button presses/messages being replayed after restart.
