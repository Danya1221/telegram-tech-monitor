...
async def get_all_dialogs(owner_id: int):
    """
    One combined chat list from both Telegram accounts.
    Accounts are read from PostgreSQL; dialogs are requested in parallel.
    """
    ...
    for row, dialogs in zip(
        ...
        for dialog in dialogs:
            item = dict(dialog)
            item["account_index"] = account_index.get(
                account_id,
                1,
            )
