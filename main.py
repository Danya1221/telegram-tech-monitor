...
async def init_db():
    ...
    async with db_pool.acquire() as conn:
        ...
        # v11: keep the original selected-chat message too.
        # Interest analytics matches against raw_text, not only against
        # the extracted analytics phrase.
        await conn.execute("""
            ALTER TABLE all_request_analytics_events
        ...
        # ================================================================
        # V12 CLEAN ANALYTICS LEDGER
        # ================================================================
        # One selected-chat Telegram message = one row. Analytics no longer
        # disappears because an extractor/dedup branch returned early.
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS chat_request_events_v2 (
                owner_id BIGINT NOT NULL,
                account_id BIGINT,
                chat_id BIGINT NOT NULL,
                message_id BIGINT NOT NULL,
        ...
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_chat_request_events_v2_owner_time
            ON chat_request_events_v2 (owner_id, created_at DESC)
        """)

        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_chat_request_events_v2_owner_request_time
            ON chat_request_events_v2 (owner_id, is_request, created_at DESC)
        """)

        # Preserve accumulated analytics from older builds. Old rows had
        # already passed the old request detector, therefore is_request=TRUE.
        await conn.execute("""
            INSERT INTO chat_request_events_v2 (
                owner_id,
                account_id,
                chat_id,
                message_id,
            ...
            SELECT
                ...
                chat_title,
                seller_key,
                COALESCE(NULLIF(raw_text, ''), found_request),
                found_request,
                brand,
                TRUE,
                created_at
...
async def consolidate_db_v13():
    """
    V13: one physical source of truth for every user-facing data branch.

    Old tables are read once as migration inputs and then dropped.
    """
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            await conn.execute(
                "SELECT pg_advisory_xact_lock(713202613)"
            )

            # 1. Notification queries.
            await conn.execute("""
                CREATE TABLE IF NOT EXISTS quantum_queries_v1 (
                    id BIGSERIAL PRIMARY KEY,
                    owner_id BIGINT NOT NULL,
                    query TEXT NOT NULL,
                    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
            ...
            """)
            await conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_quantum_queries_v1_owner
                ON quantum_queries_v1 (owner_id, id DESC)
            """)

            # 2. Analytics-only interests.
            await conn.execute("""
                CREATE TABLE IF NOT EXISTS quantum_interests_v1 (
                    id BIGSERIAL PRIMARY KEY,
                    owner_id BIGINT NOT NULL,
                    query TEXT NOT NULL,
                    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
            ...
            """)
            await conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_quantum_interests_v1_owner
                ON quantum_interests_v1 (owner_id, id DESC)
            """)

            # 3. Selected chats. account_id is part of the key.
            await conn.execute("""
            ...
            # 4. ONE analytics/event ledger for every analytics screen.
            await conn.execute("""
            ...
            # ------------------------------
            # Migrate currently active old state.
            # ------------------------------
            await conn.execute("""
                INSERT INTO quantum_queries_v1 (
                    id, owner_id, query, created_at
                )
                SELECT id, owner_id, query, created_at
            ...
            await conn.execute("""
                INSERT INTO quantum_interests_v1 (
                    id, owner_id, query, created_at
                )
                SELECT id, owner_id, query, created_at
            ...
            # init_db already folded old all-request rows into this ledger.
            await conn.execute("""
                ...
                DO UPDATE SET
                    ...
                    created_at=LEAST(
                        quantum_events_v1.created_at,
                        EXCLUDED.created_at
                    )
            """)

            # Advance explicit-ID sequences.
            for table_name in (
                'quantum_queries_v1',
            ...
            for old_table in (
                ...
                await conn.execute(
                    f'DROP TABLE IF EXISTS {old_table} CASCADE'
                )

            await conn.execute("""
                INSERT INTO app_settings (key, value)
                VALUES ('database_schema', 'v13_single_db')
                ON CONFLICT (key)
                DO UPDATE SET value=EXCLUDED.value
            """)

    print(
        'DATABASE V13 | one canonical branch: '
        'queries/interests/chats/events'
    )


async def verify_canonical_db_v13():
    """Stop startup if an obsolete branch survived the migration."""
    canonical_tables = (
        'quantum_queries_v1',
        'quantum_interests_v1',
    ...
    async with db_pool.acquire() as conn:
        for table_name in canonical_tables:
            ...
            if not exists:
                raise RuntimeError(
                    f'Canonical DB table missing: {table_name}'
                )
        ...
        if alive:
            raise RuntimeError(
                'Obsolete DB branch survived: ' + ', '.join(alive)
            )

    print(
        'DATABASE V13 | verification OK; '
        'old branches are absent'
    )
...
async def delete_query(
    ...
    async with db_pool.acquire() as conn:
        ...
        if ids_to_delete:
            await conn.execute(
                """
                DELETE FROM quantum_queries_v1
                WHERE owner_id=$1
                  AND id = ANY($2::bigint[])
                """,
                owner_id,
...
async def delete_interest_query(
    ...
    async with db_pool.acquire() as conn:
        ...
        if ids_to_delete:
            await conn.execute(
                """
                DELETE FROM quantum_interests_v1
                WHERE owner_id=$1
                  AND id = ANY($2::bigint[])
                """,
                owner_id,
...
async def migrate_legacy_selected_chats(owner_id: int):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute(
            """
            UPDATE quantum_selected_chats_v1
            SET account_id=$1
            WHERE owner_id=$2
...
def extract_all_request_for_analytics(
    ...
    independently of quantum_queries_v1.
...
async def log_chat_request_event(
    ...
    """
    Canonical analytics write. It runs for EVERY new text message from a
    selected chat. There is no seller/request dedup here: Telegram message PK
    is the only dedup. Notification dedup stays separate.
    """
    brand = detect_brand(
        request_text or raw_text,
        raw_text,
    ) if request_text else None
...
def run_core_self_tests():
    """Fast deterministic checks for the bugs that previously regressed."""
    cases = [
...
async def main():
    await init_db()
    await consolidate_db_v13()
    await verify_canonical_db_v13()
    run_core_self_tests()

    # ВАЖНО:
    # старую таблицу queries больше вообще не читаем.
    # quantum_queries_v1 — новый чистый список поиска.
