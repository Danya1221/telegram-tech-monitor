...
async def ensure_saved_account_client(
    ...
    async with lock:
        ...
        try:
            ...
            if not authorized:
                # ВАЖНО: больше НЕ ставим active=FALSE автоматически.
                # Временный сбой/рестарт не должен "удалять" аккаунт.
                await tg_client.disconnect()

                print(
                    "TELEGRAM ACCOUNT SAVED BUT NOT AUTHORIZED | "
                    f"account_id={account_id}"
                )
            ...
            # Не удаляем привязку автоматически.
            # Пользовательский аккаунт остаётся видимым в интерфейсе.
            print(
                "TELEGRAM SESSION NEEDS REAUTH | "
                f"account_id={account_id}"
            )
...
async def account_supervisor_loop(
    ...
    """
    Сохранённые Telegram-аккаунты постоянно восстанавливаются.
    Один краткий disconnect больше не требует /login.
    """
    while True:
        await asyncio.sleep(
            15
