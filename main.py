...
async def register_logged_in_client(
    ...
    # Login-client создавался только для авторизации.
    # Для 24/7 мониторинга создаём отдельный persistent client
    # с бесконечным auto-reconnect.
    try:
        await tg_client.disconnect()
    except Exception:
        pass

    old_client = clients.pop(
        account_id,
        None,
    )
    ...
    monitor_client = (
        make_persistent_monitor_client(
            session_string,
            account_id,
        )
    )
    ...
    if not await asyncio.wait_for(
        monitor_client.is_user_authorized(),
        timeout=8,
    ):
        await monitor_client.disconnect()
        raise RuntimeError(
            "Telegram session сохранилась, "
            "но monitor client не авторизован."
        )

    await monitor_client.set_receive_updates(
        True
    )
