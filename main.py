...
async def process_login_phone(
    ...
    # Важно: отвечаем ДО запроса к Telethon.
    # Если этого сообщения нет, значит update вообще попал
    # не в этот код/не в этот deployment.
    await message.answer(
    ...
    try:
        ...
        # Оставлять зависший waiting_phone после реальной ошибки
        # опасно — следующая попытка должна начинаться чисто.
        await delete_login_flow(
            owner_id
        )
        await state.clear()
...
async def process_login_code(
    ...
    try:
        ...
        # finish_persistent_login оставляет успешный client
        # подключённым как monitor client.
        tg_client = None
...
async def process_login_password(
    ...
    try:
        ...
        # При неверном 2FA flow сохраняем:
        # пользователь может просто ввести пароль ещё раз.
        await message.answer(
